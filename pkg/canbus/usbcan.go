...
func (c *USBCANChannel) parseFrames(bufAddr *[]byte) error {
	for {
		...
		if (buf[1] >> 6) == 3 {
			...
			if endByte != 0x55 {
				c.log.Debugf("Data frame with bad end byte: %v %v %X %+v EB: %X\n", extendedFrame, remoteFrame, frameID, dataBytes, endByte)
				*bufAddr = []byte{}
				return nil
			}
